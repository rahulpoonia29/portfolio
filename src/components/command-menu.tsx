...
function CommandItem({ keyKey, label }: { keyKey: string; label: string }) {
  return (
    <div className="text-muted-foreground hover:bg-muted/50 hover:text-foreground flex items-center justify-between rounded px-3 py-2 text-sm transition-colors">
      <span>{label}</span>
      <kbd className="border-border bg-muted text-muted-foreground pointer-events-none inline-flex h-5 items-center gap-1 rounded border px-1.5 font-mono text-[10px] font-medium opacity-100 select-none">
        {keyKey}
      </kbd>
    </div>
  );
